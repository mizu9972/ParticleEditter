...
ワールド変換行列を作る
--------------------------*/
void ParticleSystemMathUtil::DX11MakeWorldMatrix(XMFLOAT4X4& inoutmat, XMFLOAT3& angle, XMFLOAT3& trans)
{
...
３次元ベクトル×行列
--------------------------*/
void ParticleSystemMathUtil::DX11Vec3MulMatrix(DirectX::XMFLOAT3& outvec, const DirectX::XMFLOAT3& invec, const DirectX::XMFLOAT4X4& mat) {
...
外積を計算する
--------------------------*/
void ParticleSystemMathUtil::DX11Vec3Cross(DirectX::XMFLOAT3& outvec, DirectX::XMFLOAT3& invec1, DirectX::XMFLOAT3& invec2) {
...
内積を計算する
--------------------------*/
void ParticleSystemMathUtil::DX11Vec3Dot(float& dot , const DirectX::XMFLOAT3& invec1, const DirectX::XMFLOAT3& invec2) {
...
正規化する
--------------------------*/
void ParticleSystemMathUtil::DX11Vec3Normalize(DirectX::XMFLOAT3& outvec, DirectX::XMFLOAT3& invec) {
...
単位行列にする
--------------------------*/
void ParticleSystemMathUtil::DX11MtxIdentity(DirectX::XMFLOAT4X4& mat) {
...
平行移動行列を作成する
--------------------------*/
void ParticleSystemMathUtil::DX11MtxTranslation(const XMFLOAT3& trans, XMFLOAT4X4& matrix) {
...
行列からクオータニオンを生成する
--------------------------*/
void ParticleSystemMathUtil::DX11GetQtfromMatrix(const XMFLOAT4X4& mat, XMFLOAT4& qt) {
...
--------------------------*/
void ParticleSystemMathUtil::DX11QtRotationAxis(XMFLOAT4& qt, XMFLOAT4& axis, float angle) {
...
クオータニオンの掛け算
--------------------------*/
void ParticleSystemMathUtil::DX11QtMul(XMFLOAT4& outqt,const XMFLOAT4& qt1,const XMFLOAT4& qt2) {
...
クオータニオンから行列を生成
--------------------------*/
void ParticleSystemMathUtil::DX11MtxFromQt(XMFLOAT4X4& outmtx, const XMFLOAT4& qt){
...
　　転置する
--------------------------*/
void ParticleSystemMathUtil::DX11MtxTranspose(XMFLOAT4X4& outmtx, const XMFLOAT4X4& inmtx) {
...
クオータニオンのノーマライズ
--------------------------*/
void ParticleSystemMathUtil::DX11QtNormalize(XMFLOAT4& inqt,XMFLOAT4& outqt) {
...
ベクトルの長さを求める
--------------------------*/
void ParticleSystemMathUtil::DX11Vec4Length(XMFLOAT4& vec,float& length) {
...
ベクトルの長さを求める
--------------------------*/
void ParticleSystemMathUtil::DX11Vec3Length(XMFLOAT3& vec, float& length) {
...
２点間の距離を求める
--------------------------*/
void ParticleSystemMathUtil::DX11p2pLength(const XMFLOAT3& p1, const XMFLOAT3& p2,float& length) {
	...
	ParticleSystemMathUtil::DX11Vec4Length(vec4, length);
}

/*------------------------
スラープ補間
--------------------------*/
void ParticleSystemMathUtil::DX11QtSlerp(const XMFLOAT4& fromqt, const XMFLOAT4& toqt, float t, XMFLOAT4& ansqt) {
...
/*------------------------
  単位クオータニオンにする
--------------------------*/
void ParticleSystemMathUtil::DX11QtIdentity(XMFLOAT4& qt) {
...
--------------------------*/
void ParticleSystemMathUtil::DX11MtxRotationAxis(const XMFLOAT3& axis, float angle ,XMFLOAT4X4& outmtx) {
...
行列の掛け算
--------------------------*/
void ParticleSystemMathUtil::DX11MtxMultiply(XMFLOAT4X4& ansmtx,const XMFLOAT4X4& p1mtx, const XMFLOAT4X4& p2mtx) {
...
逆行列を求める
--------------------------*/
void ParticleSystemMathUtil::DX11MtxInverse(XMFLOAT4X4& ansmtx, const XMFLOAT4X4& mtx){
...
//==============================================================================
bool ParticleSystemMathUtil::IsInFrustum(const XMFLOAT3& pos, const XMFLOAT4X4& matrix) {

	PLANE plane[6];

	ParticleSystemMathUtil::GetPlanefromProjectionmatrix(plane, matrix);		// 行列から６平面を取り出す
...
//!	@retval	なし
//==============================================================================
void ParticleSystemMathUtil::IsInFrustum(const XMFLOAT3& pos, const XMFLOAT4X4& matrix, float ans[]) {

	PLANE plane[6];

	ParticleSystemMathUtil::GetPlanefromProjectionmatrixWithNormalize(plane, matrix);
...
//!	@retval	true フラスタム内　false フラスタム外
//==============================================================================
bool ParticleSystemMathUtil::IsInFrustum(const XMFLOAT3& pos, const XMFLOAT4X4& matrix, float ans[], PLANE* plane) {
...
//!	@retval	true フラスタム内　false フラスタム外
//==============================================================================
bool ParticleSystemMathUtil::IsInFrustum(
...
//!	@param　ビュー変換行列＊プロジェクション変換行列
//==============================================================================
void ParticleSystemMathUtil::GetPlanefromProjectionmatrix(PLANE* p, const XMFLOAT4X4& matrix) {
	// left
...
//!	@param　ビュー変換行列＊プロジェクション変換行列
//==============================================================================
void ParticleSystemMathUtil::GetPlanefromProjectionmatrixWithNormalize(PLANE* p, const XMFLOAT4X4& matrix) {
...
X軸回転行列を求める
--------------------------*/
void ParticleSystemMathUtil::DX11MtxRotationX(float angle, XMFLOAT4X4& outmtx) {
...
/*------------------------
  Y軸回転行列を求める
--------------------------*/
void ParticleSystemMathUtil::DX11MtxRotationY(float angle, XMFLOAT4X4& outmtx) {
...
/*------------------------
   Z軸回転行列を求める
--------------------------*/
void ParticleSystemMathUtil::DX11MtxRotationZ(float angle, XMFLOAT4X4& outmtx) {
...
/*------------------------
 スケーリング行列を求める
--------------------------*/
void ParticleSystemMathUtil::DX11MtxScale(float sx, float sy, float sz,XMFLOAT4X4& outmtx) {
...
　　ハルトン数列値
--------------------------*/
float ParticleSystemMathUtil::haltonseq(unsigned int no, unsigned int base) {
